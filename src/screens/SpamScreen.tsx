...
const SpamScreen = ({ navigation }: any) => {
  ...
  useEffect(() => {
    const loadSpamMessages = async () => {
      try {
        const storedSpamMessages = await AsyncStorage.getItem('spamMessages');
        if (storedSpamMessages) {
          setSpamMessages(JSON.parse(storedSpamMessages));
        }
      } catch (error) {
  ...
  return (
    <View style={styles.container}>
      {loading ? (
        <Text style={styles.loadingText}>Loading spam messages...</Text>
      ) : (
        <FlatList
          data={spamMessages}
