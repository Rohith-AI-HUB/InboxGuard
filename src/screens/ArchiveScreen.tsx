...
const ArchiveScreen = ({ navigation }: any) => {
  ...
  useEffect(() => {
    const loadArchivedMessages = async () => {
      try {
        const storedArchivedMessages = await AsyncStorage.getItem('archivedMessages');
        if (storedArchivedMessages) {
          setArchivedMessages(JSON.parse(storedArchivedMessages));
        }
      } catch (error) {
  ...
  return (
    <View style={styles.container}>
      {loading ? (
        <Text style={styles.loadingText}>Loading archived messages...</Text>
      ) : (
        <FlatList
